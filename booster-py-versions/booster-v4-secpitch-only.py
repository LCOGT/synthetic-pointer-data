...
def make_features_and_labels(split_data):
    ...
    Inputs X:
      ...
      sin(roll), cos(roll), sin(pitch), cos(pitch), 1/cos(pitch),
      previous_acq_error_ra, previous_acq_error_dec
    ...
    axis-aligned splits. 1/cos(pitch) is included explicitly since several
    TPoint terms (ch, tfh) use it directly and it blows up sharply near the
    horizon limit. tan(pitch) was tested separately and found to add no
    value (0.0 feature importance) since it's fully redundant with
    sin(pitch)/cos(pitch) already present, so it is intentionally left out
    here.
...
#TRAIN MODEL WITH THESE PARAMETERS --> VERSION 3
# XGB_PARAMS = dict(
#     booster="gbtree",
