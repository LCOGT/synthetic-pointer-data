...
def make_features_and_labels(split_data):
    ...
    axis-aligned splits. tan(pitch) and 1/cos(pitch) are added explicitly
    since several TPoint terms (mah, meh, nP, ch, tfh) use them directly and
    they blow up sharply near the horizon limit — giving the model the ratio
    directly avoids it having to reconstruct sin(pitch)/cos(pitch) through
    splits on two separate features.
